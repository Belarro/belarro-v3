...
} from '\var\task\Downloads\Claude Code\belarro-v3\frontend/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\Downloads\Claude Code\belarro-v3\frontend/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\Downloads\Claude Code\belarro-v3\frontend')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\Downloads\Claude Code\belarro-v3\frontend',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\Downloads\Claude Code\belarro-v3\frontend\.netlify\dist\run\handlers\server.js')
